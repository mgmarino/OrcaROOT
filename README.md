...
4. (SEE [Bindings](Bindings) for info on using python bindings instead of
having to build a new application.) Set up a directory for your own work. Use
the files:
...
- `ORVTreeWriter`: virtual base class for processors that write data
  to `TTree`s -- automates the `TTree` building, filling (if desired), and
