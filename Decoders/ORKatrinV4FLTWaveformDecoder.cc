// ORKatrinV4FLTWaveformDecoder.cc
...
ORKatrinV4FLTWaveformDecoder::ORKatrinV4FLTWaveformDecoder() { fDataRecord = NULL; fWaveformLength = -1; }

bool ORKatrinV4FLTWaveformDecoder::SetDataRecord(UInt_t* dataRecord) 
{
  fDataRecord = dataRecord;
  fWaveformLength = (  LengthOf(fDataRecord) / (kWaveformLength/2)  )  * 2048;//this sets GetWaveformLen() -tb-//TODO: check it -tb-
...
bool ORKatrinV4FLTWaveformDecoder::IsValid() 
{ 
...
void ORKatrinV4FLTWaveformDecoder::DumpBufferHeader()
{
  if(fDataRecord)
  {
...
size_t ORKatrinV4FLTWaveformDecoder::CopyWaveformData( UShort_t* waveform, 
                                                     size_t len )
...
size_t ORKatrinV4FLTWaveformDecoder::CopyWaveformDataDouble(double* waveform, size_t len)
//copies the waveform data to the array pointed to by
//waveform, which is of length len
{
  size_t wflen = GetWaveformLen();
...
void ORKatrinV4FLTWaveformDecoder::Dump(UInt_t* dataRecord) //debugging 
{
  ORLog(kDebug) << std::endl << std::endl << "ORKatrinV4FLTWaveformDecoder::Dump():" << std::endl ;
